		...
		if business then
			if _bizPhones[business] and _bizPhones[business].call then
				...
				if caller ~= nil and _calls[caller] ~= nil then
					plsr.Logger:Trace("Phone", string.format("%s Ending Call With %s", business, caller))
					...
					plsr.Phone.Call:CreateRecord(_calls[caller].data)
					plsr.Phone.Notification:RemoveById(caller, "PHONE_CALL")
					plsr.VOIP.Phone:SetCall(caller, 0)
					_calls[caller] = nil
				else
					plsr.Logger:Trace(
						"Phone",
				...
				if _bizPhones[business].call.handler then
					plsr.VOIP.Phone:SetCall(_bizPhones[business].call.handler, 0)
			...
			if _calls[source] ~= nil then
				...
				plsr.Phone.Call:CreateRecord(_calls[source].data)
				...
				plsr.Phone.Notification:RemoveById(source, "PHONE_CALL")
				plsr.VOIP.Phone:SetCall(source, 0)
	
				if _calls[source].isBiz then
					if _bizPhones[_calls[source].isBiz].call and _bizPhones[_calls[source].isBiz].call.handler then
						plsr.VOIP.Phone:SetCall(_bizPhones[_calls[source].isBiz].call.handler, 0)
					...
					if _calls[source].target ~= nil and _calls[_calls[source].target] ~= nil then
						plsr.Logger:Trace("Phone", string.format("%s Ending Call With %s", source, _calls[source].target))
						...
						plsr.Phone.Call:CreateRecord(_calls[_calls[source].target].data)
						plsr.Phone.Notification:RemoveById(_calls[source].target, "PHONE_CALL")
						plsr.VOIP.Phone:SetCall(_calls[source].target, 0)
...
AddEventHandler("Characters:Server:PlayerLoggedOut", function(source, cData)
	if _calls[source] ~= nil then
		plsr.Phone.Call:End(source)
	end

	if _bizCallHandlers[source] ~= nil then
		plsr.Phone.Call:End(-1, _bizCallHandlers[source])
	end
end)
...
AddEventHandler("Phone:Server:RegisterCallbacks", function()
	plsr.Callbacks:RegisterServerCallback("Phone:Phone:CreateCall", function(src, data, cb)
		local char = plsr.Fetch:CharacterSource(src)
		if _calls[src] == nil and char:GetData("Phone") ~= data.number then
			local callingContact = plsr.Phone.Contacts:IsContact(char:GetData("SID"), data.number)
			...
			_calls[src] = {
				...
				data = {
					...
					decryptable = hasValue(char:GetData("States") or {}, "PHONE_VPN"),
					unread = false,
				},
			}
			plsr.Phone.Notification:AddWithId(src, "PHONE_CALL", "Dialing", callingStr, os.time(), -1, "phone", {
			...
			if isBusiness and _bizPhones[isBusiness] then
				...
				if bizData.call then -- Already Busy
					plsr.Phone.Call:CreateRecord(callerData)
			...
			local target = plsr.Fetch:CharacterData("Phone", data.number)
			if target ~= nil and hasValue(target:GetData("States"), "PHONE") then
				if _calls[target:GetData("Source")] == nil then
					cb(true)

					plsr.Logger:Trace("Phone", string.format("%s Starting Call With %s", src, target:GetData("Source")))

					_calls[src].target = target:GetData("Source")

					local destContact = plsr.Phone.Contacts:IsContact(target:GetData("SID"), char:GetData("Phone"))
					local destStr = char:GetData("Phone")
					if destContact then
					...
					_calls[target:GetData("Source")] = {
						...
						data = {
							...
							limited = data.limited,
							anonymous = data.isAnon,
							decryptable = hasValue(target:GetData("States") or {}, "PHONE_VPN"),
							unread = false,
						},
					}
					...
					plsr.Phone.Call:CreateRecord(recipData)
	...
	plsr.Callbacks:RegisterServerCallback("Phone:Phone:AcceptCall", function(src, data, cb)
		local char = plsr.Fetch:CharacterSource(src)
		...
		if _calls[src] ~= nil then
			if _calls[src].isBiz and _bizPhones[_calls[src].isBiz].call then
				...
				plsr.VOIP.Phone:SetCall(src, cid)
				plsr.VOIP.Phone:SetCall(bizCallerSource, cid)
			elseif _calls[src].target ~= nil and _calls[_calls[src].target] ~= nil then
				...
				plsr.VOIP.Phone:SetCall(src, cid)
				plsr.VOIP.Phone:SetCall(_calls[src].target, cid)
			end
		else
			plsr.Logger:Trace(
				"Phone",
	...
	plsr.Callbacks:RegisterServerCallback("Phone:Phone:EndCall", function(src, data, cb)
		plsr.Phone.Call:End(src)
	end)

	plsr.Callbacks:RegisterServerCallback("Phone:Phone:ReadCalls", function(src, data, cb)
		local char = plsr.Fetch:CharacterSource(src)
		if char ~= nil then
			plsr.Phone.Call:Read(char:GetData("Phone"))
		end
	end)

	plsr.Callbacks:RegisterServerCallback("Phone:MuteBiz", function(source, id, cb)
		local char = plsr.Fetch:CharacterSource(source)
		if char ~= nil and id and _bizPhones[id] and plsr.Jobs.Permissions:HasJob(source, _bizPhones[id].job) then
	...
	plsr.Callbacks:RegisterServerCallback("Phone:DeclineBizCall", function(source, id, cb)
		local char = plsr.Fetch:CharacterSource(source)
		if char ~= nil and id and _bizPhones[id] and _bizPhones[id].call and plsr.Jobs.Permissions:HasJob(source, _bizPhones[id].job) then
			plsr.Phone.Call:End(-1, id)
		else
			cb(false)
		end
	end)

	plsr.Callbacks:RegisterServerCallback("Phone:AcceptBizCall", function(source, id, cb)
		local char = plsr.Fetch:CharacterSource(source)
		if char ~= nil and not _calls[source] and id and _bizPhones[id] and _bizPhones[id].call and not _bizPhones[id].call.outgoing and plsr.Jobs.Permissions:HasJob(source, _bizPhones[id].job) then
			if not _bizPhones[id].call.handler then
				...
				plsr.VOIP.Phone:SetCall(source, cid)
				plsr.VOIP.Phone:SetCall(targetSource, cid)
	...
	plsr.Callbacks:RegisterServerCallback("Phone:MakeBizCall", function(source, data, cb)
		local char = plsr.Fetch:CharacterSource(source)
		if char ~= nil and data and data.id and data.number and _bizPhones[data.id] and not _bizPhones[data.id].call and plsr.Jobs.Permissions:HasJob(source, _bizPhones[data.id].job) and data.number ~= char:GetData("Phone") then
			local target = plsr.Fetch:CharacterData("Phone", data.number)
			if target ~= nil and hasValue(target:GetData("States"), "PHONE") then
				if _calls[target:GetData("Source")] == nil then
					cb(true)
					...
					plsr.Logger:Trace("Phone", string.format("%s Starting Call With %s", data.id, target:GetData("Source")))

					local destContact = plsr.Phone.Contacts:IsContact(target:GetData("SID"), _bizPhones[data.id].number)
