...
AddEventHandler("Phone:Server:RegisterCallbacks", function()
	plsr.Callbacks:RegisterServerCallback("Phone:Message:InitLoad", function(source, data, cb)
	...
	plsr.Callbacks:RegisterServerCallback("Phone:Messages:LoadTexts", function(source, data, cb)
		local char = plsr.Fetch:CharacterSource(source)
		if char ~= nil then
			local messages = MySQL.rawExecute.await(
	...
	plsr.Callbacks:RegisterServerCallback("Phone:Messages:SendMessage", function(source, data, cb)
		local char = plsr.Fetch:CharacterSource(source)
		local data2 = {
			owner = data.number,
			number = data.owner,
		...
		if id then
			local targetChar = plsr.Fetch:CharacterData("Phone", data.number)
			if targetChar ~= nil then
				data2.id = id.insertId + 1
				data2.contact = plsr.Phone.Contacts:IsContact(targetChar:GetData("SID"), data2.number)
	...
	plsr.Callbacks:RegisterServerCallback("Phone:Messages:ReadConvo", function(source, data, cb)
		local char = plsr.Fetch:CharacterSource(source)
		if char ~= nil then
			plsr.Phone.Messages:Read(char:GetData("Phone"), data)
		end
	end)

	plsr.Callbacks:RegisterServerCallback("Phone:Messages:DeleteConvo", function(source, data, cb)
		local char = plsr.Fetch:CharacterSource(source)
		if char ~= nil then
			plsr.Phone.Messages:Delete(char:GetData("Phone"), data)
			cb(true)
		else
			cb(false)
		end
